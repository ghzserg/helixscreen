...
**Total Hidden Tests:** 21 (+ 1 unhidden test that hangs)
...
2. **Fix G-code geometry cache issues** (2 tests)
3. **Set up UI integration test infrastructure** (8 tests)
4. **Fix WiFi scan_once() async behavior** (1 test)

### LOW Priority

5. **Document slow/benchmark tests** - working as designed
6. **Review state machine test expectations** - may need behavior update
